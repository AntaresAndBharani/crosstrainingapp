...
### Fixed
- **Release Workflow Error Annotation Trailing Exit 1 Assertions & Test Equality Null Safety**: Extended static regex assertions in `scripts/tests/ReleaseWorkflow.Static.Tests.ps1` to require each hardened guard's `::error::` annotation in `.github/workflows/release.yml` to be immediately followed by `exit 1`, and improved `Assert-Equal` in `scripts/tests/TestHelpers.ps1` with safe left-side null comparisons across PowerShell Core platforms (#313, #319, #334).
