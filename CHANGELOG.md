...
- **Resilient Multi-Delimiter Parsing, Inline Colon Guard & Triset Boundary Enforcement (`WorkoutDocumentParser`)**: Hardened `WorkoutDocumentParser` in `com.fractanomics.crosstraining.util` to eliminate phantom exercise creation and movement name pollution when parsing complex workout notes; added an Inline Colon Guard in `isProbableSetsLine` ensuring lines with non-label prefixes before colons (e.g. `Front Squat: 100 100 100 100`) are never misclassified as sets lines; enforced context-aware cluster boundary termination in `parseDocument` stopping triset ingestion when established numbered/bulleted sequences are broken or when modality counts (3 for triset, 2 for superset) are reached followed by unindented movements or format keywords; implemented blank-line skipping during block sets lookahead and triset pairing (`while (peek.isBlank()) peek++`) and added an orphan sets binding invariant; sanitized movement names by stripping list prefixes (`1- `, `2- `), trailing sets/weights boilerplate with word-boundary protection (`\bweights?`, `\bsets?`) preserving compound words like `"Bodyweight"`, and updated `TARGET_REPS_SUFFIX_REGEX` to consume `"xN reps"` completely; backed by comprehensive unit test suite in `WorkoutDocumentParserTest` covering all Gherkin scenarios (#523).
- **Workout Journey Setup Assistant UI & Downstream SessionEditor Integration**: Implemented `WorkoutJourneyAssistantSheet` in `com.fractanomics.crosstraining.ui.components` providing a 4-step Material 3 modal bottom sheet wizard for rapid end-to-end workout ingestion; integrated `SheetState` dismissal guard (`confirmValueChange`) active during Steps 2–4 with exit confirmation alert dialog preventing accidental loss of in-flight review data; implemented Step 1 raw workout notes pasting and AI document parser invocation, Step 2 routine template/session options and cycle assignment, Step 3 in-flight exercise grounding review with editable category and tracking metric dropdowns for proposed new exercises, and Step 4 macro-block preview and atomic Room persistence; extended `AppViewModel` with reactive state flow `workoutJourneyDraft`, `processWorkoutText`, `setJourneyStep`, `updateJourneyTemplateOptions`, `updateMissingExerciseConfig`, `confirmWorkoutJourney`, and `clearWorkoutJourneyDraft`; updated `SessionEditor.kt` with visual macro-block section banner grouping and preservation through `BlockDraft` and `BlockSeed` preventing downstream metadata eviction, and added Macro-Block Section input field inside `CompactBlockEditor`; wired direct "Paste Workout Notes" entry point into `SessionEditor` TopAppBar / `LogSessionScreen` and "Import from Notes" filter chip into `LibraryScreen` (Routines tab); backed by comprehensive unit test suites in `WorkoutJourneyAssistantViewModelTest` and `SessionEditorSectionParityTest` (#517).
- **Entity Grounding, Composite Complex Isolation & Batch Ingestion Engine (`WorkoutEntityResolver`)**: Implemented `WorkoutEntityResolver` in `data.ai` for in-transaction case-insensitive name deduplication against the local SQLite Exercise catalog; segregated movements into existing library matches versus missing exercise proposals; added deterministic category and metric type inference heuristics covering barbell, accessory/dumbbell, machine, and gymnastics modalities; isolated barbell complexes by registering a distinct composite `Exercise` entity (e.g. `Clean + Hang Clean + Front Squat + Push to OverHead`) to preserve dedicated 1RM progression curves without corrupting component lifts, while linking component IDs in `SessionBlock.exerciseIdsCsv`; implemented atomic `Repository.persistWorkoutJourney` executing within `withDatabaseTransaction` to catalog deduplicated missing exercises, create optional `Routine` templates with section-attributed `RoutineBlock`s, and persist `Session` with section-grouped `SessionBlock`s and `BlockSet`s with full rollback on failure; backed by comprehensive unit and database integration test suites in `WorkoutEntityResolverTest` and `WorkoutJourneyRepositoryTest` (#516).
- **Resilient Multi-Block & Syntax-Aware Document Parser (`WorkoutDocumentParser`)**: Implemented a pure Kotlin, deterministic, zero-latency document parser in `WorkoutDocumentParser.kt` running locally on `Dispatchers.Default` (< 15ms) with zero Android platform dependencies; normalized European decimal commas using strict digit-bounded lookaround regex `(?<=\d),(?=\d)` preserving prose and list punctuation; extracted routine titles, repeatable tags (`repeatable`/`repeateble`), and macro-block section headers (`... block`); identified barbell complexes (`Clean + Hang Clean...`) with component movement decomposition and interval trisets (`E3MOM Trisets`, `E2,5MOM Trisets`) with cluster scheme grouping (`scheme = "TRISET_N"`); parsed shorthand `weight(reps)` grammar (`0(4)`, `0(5 reps)`, `60(1 rep)`, `60(fail)`, `not_done`), tracking partial reps and failed attempts while skipping omitted sets; resolved accessory round counts via anchor movement inheritance in interval trisets; backed by a comprehensive unit test suite in `WorkoutDocumentParserTest` covering all Gherkin scenarios and edge cases (#515).
- **Room Database Schema v7, Data Models & Backup CSV v4 Roundtrip Parity**: Upgraded `AppDatabase` schema to version 7 introducing `section: String` on `routine_blocks`, and `section: String` and `exerciseIdsCsv: String` on `session_blocks` with default empty string values; implemented and registered `MIGRATION_6_7` in `AppDatabase.build()` and `AppDatabase.demo()`; generated and verified Room schema artifact `app/schemas/com.fractanomics.crosstraining.data.AppDatabase/7.json`; updated UI draft models (`BlockDraft`, `BlockSeed`, `BlockState`) in `SessionDraft.kt` and `SessionEditor.kt` preserving `section` and `exerciseIdsCsv` without data loss; upgraded `BackupData` snapshot and `BackupCsv` to header format `#crosstraining-backup-v4` serializing/deserializing `#routineBlocks` and new `#blocks` columns with backward-compatible parsing of legacy v1/v2/v3 backups; backed by instrumented migration verification in `AppDatabaseMigrationTest` and JVM unit tests in `RoomSchemaExportTest` and `BackupCsvV4AndDraftParityTest` (#514).

### Changed
- **Session-Scoped In-Memory DataModeManager & Background Sync Isolation**: Converted `DataModeManager` to an in-memory, session-scoped data mode model unconditionally defaulting to Real Data (`demoMode = false`) on every app launch, eliminating `KEY_DEMO_MODE` reading and persistence in `SharedPreferences`; exposed `val realRepository: Repository` as `testRepository ?: underlyingRealRepository` for safe cloud sync access and JVM unit testability; removed forced `viewModel.setDemoMode(true)` on guest entry in `AppNavigation.kt`; structurally isolated background cycle synchronization calls (`saveCycle`, `saveCycleWithGoals`, `deleteCycleGoal`) in `AppViewModel.kt` to pass `data.realRepository` unconditionally, preventing demo fixtures from reaching Cloud Firestore; backed by comprehensive unit tests in `DataModeManagerTest` and `AppViewModelCycleSyncIsolationTest` (#482).
...
### Added
- **Presentation Layer UI (ProgressScreen, BottomSheet & SummaryCard)**: Implemented athlete body weight tracking UI featuring `WeightSummaryCard` displaying current weight, delta percentage badge, 7-day moving average, and an interactive unit toggle pill between `kg` and `lbs`; added `WeightEntryBottomSheet` for modal entry logging and editing with decimal input, date quick-pick chips (Today/Yesterday) and `DateField` datepicker, optional notes, canonical range validation, and delete affordance; refactored `ProgressScreen.kt` hoisting mode filter chips above empty states, adding `ProgressMode.BODY_WEIGHT` unconditionally, promoting `progressMode` to `rememberSaveable`, adding mode-conditional FAB in `Scaffold`, and integrating `WeightOverviewContent` with timeframe filtering (`7D`, `30D`, `90D`, `1Y`, `All`), `MultiLineChart` dual-series rendering, chronological history list, and soft-delete with Undo Snackbar restoration; added persistent `weightUnit` preference in `DataModeManager` and reactive state flows (`weightEntries`, `weightUnit`) and actions in `AppViewModel`; backed by comprehensive JVM unit test suite in `AppViewModelWeightTest` (#506).
- **Cloud Synchronization with Tombstone-Aware Merge & Firestore Security Rules**: Integrated `weight_entries` collection into `UserCloudSyncManager` with bi-directional synchronization under `environments/{currentEnv}/users/{uid}/data/weight_entries`; implemented pre-flight tombstone purging removing local tombstones older than 90 days (`cutoff = System.currentTimeMillis() - 90L * 24 * 60 * 60 * 1000L`) before cloud operations; enabled concurrent upload in `supervisorScope` evaluating overwrite guards with tombstone-inclusive payloads (`isLocallyEmpty = weightPayload.isEmpty()`) and preserving soft-deleted records; implemented tombstone-aware last-write-wins date merge on download comparing `maxOf(updatedAtMillis, deletedAtMillis ?: 0L)` between local and remote snapshots and batch-upserting winners via `Repository.importWeightEntries`; extended dual-read cloud migration predicates (`isNewUidEmpty`, `hasLegacyData`) and legacy swap assignment to include weight history; documented owner-only Firestore security rule in `docs/draft-requisites/firestore-rules.md`; backed by comprehensive JVM unit tests in `WeightCloudSyncTest` covering merge conflict resolution, pre-flight purges, tombstone publishing, and legacy migration (#505).
- **Domain Analytics Engine & Gap-Aware Chart Component**: Implemented pure Kotlin domain analytics engine in `WeightAnalytics.kt` providing timeframe filtering (`7D`, `30D`, `90D`, `1Y`, `All`), closed calendar interval `[t-6, t]` Simple Moving Average calculation (`SMA-7`) with `N >= 3` sample gating and null emission for sparse intervals, paired uniform index decimation capping long-term datasets to $\le 120$ points while strictly preserving `raw.size == sma.size` parity and boundary points, and canonical physiological weight range validation ($20.0 \le \text{weightKg} \le 350.0$ and derived imperial bounds); widened `ChartPoint.value` in `LineChart.kt` to `Float? = null` with backward-compatible non-null construction, safe `mapNotNull` min/max and scale evaluation preventing NaN errors, clean polyline path breaking across null intervals, and null-dot skipping; backed by comprehensive JVM unit test suites in `WeightAnalyticsTest` and `LineChartGapAwareTest` (#504).
- **Room Database Schema v6, Backup CSV v3 & Weight Snapshot Lifecycle**: Upgraded `AppDatabase` schema to version 6 introducing the `WeightEntry` entity with natural primary key `date: LocalDate`, nullable `deletedAtMillis: Long?` for soft-deletion, and `WeightDao` providing `@Upsert` capabilities, active record filtering (`deletedAtMillis IS NULL`), and tombstone purging; implemented and registered `MIGRATION_5_6` in both `AppDatabase.build()` and `AppDatabase.demo()`; generated and verified schema artifact `app/schemas/com.fractanomics.crosstraining.data.AppDatabase/6.json`; extended `BackupData` and upgraded `BackupCsv` to header format `#crosstraining-backup-v3` with backwards-compatible parsing of legacy v1/v2 backups; integrated weight snapshots into `Repository.exportSnapshot()` and `Repository.importSnapshot()` with atomic table clearing and upserts; bumped `DemoData.SEED_VERSION` to 4 provisioning 30 days of realistic weight history; backed by instrumented migration verification in `AppDatabaseMigrationTest` and JVM unit tests in `RoomSchemaExportTest`, `BackupCsvWeightTest`, and `RepositoryWeightSnapshotTest` (#503).
- **Schema Export Infrastructure & Room v5 Baseline Generation**: Configured Room schema export via KSP argument `room.schemaLocation` pointing to `$projectDir/schemas` in `app/build.gradle.kts`; enabled `exportSchema = true` on `AppDatabase` at database version 5; added `androidx-room-testing` and `androidx-test-runner` dependencies to `gradle/libs.versions.toml` and the `androidTest` configuration; generated and committed the verified baseline Room schema artifact `app/schemas/com.fractanomics.crosstraining.data.AppDatabase/5.json` for downstream migration testing; backed by automated JVM unit tests in `RoomSchemaExportTest` and androidTest compilation verification in `RoomTestingDependencyAvailabilityTest` (#502).
- **Actionable Card-Level Recovery UI & Debounced Retry**: Extracted stateless `CloudBackupSyncCard` and `LegacySessionReauthBanner` in `ProfileScreen.kt` adhering to `.agents/rules/03_compose_ui_standards.md` §1; introduced an actionable inline error recovery container displaying `CloudSyncErrorMapper.SESSION_EXPIRED_MESSAGE` ("Session expired. Please sign in again to back up your workouts") when cloud sync fails due to expired or unauthenticated sessions; added prominent [Sign In Again] action invoking `androidx.credentials.CredentialManager` (`signInWithGoogleCredential`) directly with seamless fallback; implemented 5-second cooldown debounce state (`isCooldownActive`) following sync failures disabling the "Sync Now" button to prevent rapid UI hammering; added security re-auth banner notifying athletes with detected legacy email sessions ("Security update: Please sign in again to connect your cloud account") that clears upon successful authentication or dismissal; accompanied by Compose UI instrumented tests in `ProfileScreenSyncRecoveryTest` and JVM unit tests in `ProfileScreenRecoveryUnitTest` (#496).
- **Fault-Isolated Concurrent Upload (supervisorScope) & Error Reporting**: Refactored `UserCloudSyncManager.uploadUserData` to run `repo.cleanupDuplicateRoutines()` as an explicit synchronous pre-flight step prior to collection uploads, guaranteeing routine deduplication before database extraction; decoupled collection uploads (`exercises`, `routines`, `sessions`, `cycle_goals`, `rep_maxes`) into independent concurrent child tasks inside a `supervisorScope` with per-document error tracking so transient network timeouts or failures on an individual collection do not cancel sibling uploads; aggregated per-collection outcomes into the overall upload result; added `resetSyncStatus()` and exposed `lastSyncError: StateFlow<String?>` in `AppViewModel` ensuring `syncState` resets to `IDLE` before each new sync attempt launches; accompanied by comprehensive unit and integration test coverage in `TokenBoundIdentitySyncTest` verifying pre-flight dedup sequencing, `supervisorScope` sibling fault isolation, IDLE state reset, and reactive error reporting (#495).
- **Cold-Start Auth State Await & Dual-Read Cloud Migration**: Implemented cooperative asynchronous Firebase Auth token restoration (`awaitAuthState`) in `UserCloudSyncManager` utilizing `AuthStateListener`, `CompletableDeferred`, and `withTimeoutOrNull(3000L)` to eliminate cold-start race conditions when triggering cloud synchronization immediately on profile screen entry; updated `verifyTokenBinding` to await token resolution before failing closed and emit resolved users to `_userState` / `authUser`; added `asyncAuthUidProviderForTesting` seam for deterministic JVM testing of asynchronous token arrival and timeouts; implemented dual-read migration fallback in `downloadUserData` and `recoverAllCloudRoutines` that detects empty user directories under `users/{newUid}`, queries legacy email paths (`users/{email}` and normalized email paths), imports historical workouts, routines, sessions, cycle goals, and rep maxes into Room, and re-uploads them directly to `users/{newUid}` ensuring zero data loss during cloud identity migrations; backed by comprehensive unit tests in `TokenBoundIdentitySyncTest` covering auth await resolution, timeout handling, and dual-read cloud migration (#494).
- **Upstream UID Sanitization & Legacy Re-Auth Startup Migration**: Enforced strict non-email UID validation at write time in `DataModeManager.saveAuthSession` gracefully rejecting malformed or email-based UID strings (containing `@` or blank) without persisting corrupted state to `SharedPreferences`; implemented cold-start migration in `AppViewModel.init` detecting legacy email UIDs, clearing malformed sessions from persistent storage, and exposing an observable `legacySessionRequiresReauth: StateFlow<Boolean>` that resets to `false` upon successful authentication (`signUpWithEmail`, `logInWithEmail`, `logInWithGoogle`, `logInWithGoogleAccount`), sign out, or `SyncStatus.SUCCESS`; ensured `verifyTokenBinding` does not fail closed with unhandled exceptions on cleared sessions; accompanied by comprehensive unit test coverage in `TokenBoundIdentitySyncTest` and `DataModeManagerTest` verifying legacy session migration, UID sanitization, fabricated UID handling, and reauth reset lifecycles (#493).
- **Token-Bound Identity Alignment, Scoped Recovery & Resilient Sync Now Execution**: Implemented strict token-bound identity alignment in `UserCloudSyncManager` enforcing that authenticated non-anonymous sessions match the underlying Firebase Auth UID prior to cloud synchronization, failing closed with a clear sign-in prompt on token mismatch or session loss; introduced injectable `authUidProviderForTesting` seam for deterministic JVM testing without live Firebase Auth; migrated Google Sign-In flows in `LoginWelcomeScreen` and `ProfileScreen` (`AuthDialog`) from deprecated Android `AccountManager` to `androidx.credentials.CredentialManager` and `GetGoogleIdOption`, wiring ID tokens directly to `UserCloudSyncManager.signInWithGoogleCredential` and handling cancellation gracefully without error popups; configured `default_web_client_id` in `strings.xml`; scoped `recoverAllCloudRoutines` strictly to the user's document path `userDoc(currentUserId)` preventing unscoped cross-tenant collection group queries with `recoverRoutinesReaderForTesting` seam; introduced `CloudSyncErrorMapper` shielding raw Firestore exception codes (e.g. `PERMISSION_DENIED`, `UNAVAILABLE`) and translating network timeouts to friendly local-storage notifications ("Network unavailable. Your data is saved locally on this device") and guest sync attempts to auth modal prompts ("Please sign in to back up workouts to the cloud"); backed by comprehensive test suites in `TokenBoundIdentitySyncTest`, `CloudSyncErrorMapperTest`, and updated contracts in `CrossAuthSignInTest` (#486).
- **Cloud Sync Structural Routing & Per-Document Overwrite Protection**: Isolated all authentication and cloud synchronization workflows (`signUpWithEmail`, `logInWithEmail`, `logInWithGoogle`, `logInWithGoogleAccount`, `triggerCloudSync`, `recoverCloudRoutines`) in `AppViewModel` to strictly target `data.realRepository`, guaranteeing that demo and test fixtures never sync with or contaminate production Firebase Cloud Firestore; implemented granular empty overwrite protection in `UserCloudSyncManager.uploadUserData` (`uploadCollectionWithGuard`) inspecting remote collection existence before executing `.set()` updates across `exercises`, `routines`, `sessions`, `cycle_goals`, and `rep_maxes` to prevent blank initializations from obliterating preexisting cloud backups; introduced disaggregated `CloudSyncResult` reporting in `AppViewModel.triggerCloudSync` and `ProfileScreen` with granular upload and download error propagation and sync status error state preservation; backed by comprehensive unit test suites in `AppViewModelCloudSyncStructuralRoutingTest` and `UserCloudSyncManagerOverwriteGuardTest` (#485).
- **Presentation Layer Data Mode Switching (ProfileCard, DrawerRow & Library Cleanup)**: Implemented stateless `DataModeCard` in `ProfileScreen.kt` adhering to `.agents/rules/03_compose_ui_standards.md` §1 providing a dedicated toggle between Real Data (crosstraining.db) and Demo Data (crosstraining-demo.db) with a status badge, clear description, and conditional "Reset Demo Data" action; added `DataModeDrawerRow` in `AppNavigation.kt` (`AppDrawerContent`) under the role switcher for rapid data mode toggling from the navigation drawer; removed redundant "Try demo data / Switch to my data" dropdown toggle from `LibraryScreen.kt` while preserving the "Reset demo data" action when demo mode is active; backed by JVM unit tests in `AppViewModelDataModeSwitchingTest` and instrumented Compose UI tests in `DataModeCardComposeTest` (#484).
- **Startup Training Cycle Provisioning for Fresh & Upgraded Production Installs**: Implemented automated startup verification in `AppDatabase` (`provisionDefaultCycleIfNeeded`, registered in `onCreate` and `onOpen` callbacks) and `Repository` (`provisionDefaultCycleIfNeeded`, `ensureDefaultCycleProvisioned`) that checks `cycleDao.getAllOnce().isEmpty()` and automatically provisions a default active training cycle named "General Training" (`isActive = true`, `startDate = LocalDate.now()`); wired reactive Flow initialization on `Repository.cycles` and `Repository.activeCycle` via `flow { provisionDefaultCycleIfNeeded(); emitAll(...) }` and `AppViewModel` initialization, guaranteeing that first-run athletes on production builds (where sample cycles are skipped) as well as existing installs upgraded from v3.0.147 can immediately log and save workouts without encountering "Create and select a cycle first"; fixed `Repository.withDatabaseTransaction` to prevent null-return fallthrough to uninitialized Room executors; updated Maestro E2E test flow `03_history_and_search_flow.yaml` to be fully self-contained on a clean real database; backed by comprehensive unit and integration tests in `StartupCycleProvisioningTest` (#483).
