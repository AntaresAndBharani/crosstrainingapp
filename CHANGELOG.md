...
### Added
- **Living Architecture Standards & Guidelines**: Authoritatively documented system overview, technology stack matrix, 4-layer clean architecture boundaries, directory and package layout, design patterns (UDF, dynamic dual-database routing, application-scoped shared TimerEngine provider, graceful foreground teardown controller, resilient numeric input sanitization, domain rule-based parsing engine), architectural constraints, and anti-pattern reference matrix in `.graph/architecture.md`.
