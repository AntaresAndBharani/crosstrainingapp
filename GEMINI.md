...
- **Build Snapshot APK:** `.\gradlew.bat assembleSnapshot --no-daemon`
...
3. **Testing:** Never break unit test suite. All new features/fixes must be verified via `testDebugUnitTest`.
4. **Local APK Sync:** When building APKs, automatically copy the output APK to `local_test\latest.apk`.
